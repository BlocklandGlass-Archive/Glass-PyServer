...
# Originally from https://github.com/williamsjj/txamqp-helpers/blob/master/txamqp_helpers/amqp.py, modified for BlocklandGlass by Nullable/dontcare4free
...
class AmqpProtocol(AMQClient):
    ...
    def _channel_open(self, arg):
        ...
        for l in self.factory.read_list:
            self.setup_read(*(l + self.factory.read_list[l]))
    ...
    def read(self, exchange, routing_key, callback, no_ack=True):
        """Add an exchange to the list of exchanges to read from."""
        if self.connected:
            # Connection is already up. Add the reader.
            self.setup_read(exchange, routing_key, callback, no_ack)
        else:
            # Connection is not up. _channel_open will add the reader when the
            # connection is up.
            pass

    def stop_reading(self, exchange, routing_key, callback):
        tup = (exchange, routing_key, callback)
        if self.connected and tup in self.consumer_tags:
            self.chan.basic_cancel(self.consumer_tags[tup])
            self.consumer_tags[tup] = None
    ...
    @inlineCallbacks
    def setup_read(self, exchange, routing_key, callback, no_ack=True):
        """This function does the work to read from an exchange."""
        queue = exchange  # For now use the exchange name as the queue name.
        consumer_tag = "%s %s" % (exchange, routing_key)  # Use the exchange name for the consumer tag for now.
        self.consumer_tags[exchange, routing_key, callback] = consumer_tag
...
class AmqpFactory(protocol.ReconnectingClientFactory):
    ...
    def read(self, exchange=None, routing_key=None, callback=None, no_ack=True):
        ...
        self.read_list[exchange, routing_key, callback] = no_ack

        # Tell the protocol to read this if it is already connected.
        if self.p != None:
            self.p.read(exchange, routing_key, callback, no_ack)

    def stop_reading(self, exchange, routing_key, callback):
        del self.read_list[exchange, routing_key, callback]

        if self.p != None:
            self.p.stop_reading(exchange, routing_key, callback)
